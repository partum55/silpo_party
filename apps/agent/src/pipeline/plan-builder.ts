...
import { emptyPlan, type CatalogProduct, type PartyPlanDraft, type VerifiedProduct } from "../domain/plan.ts";
...
export function addProducts(plan: PartyPlanDraft, additions: VerifiedProduct[]) {
...
export function findPlanProducts(plan: PartyPlanDraft, target: string) {
 ...
 */
export function removeRows(plan: PartyPlanDraft, rows: VerifiedProduct[], { exclude = true }: { exclude?: boolean } = {}) {
  ...
  return recalculate({
    ...
    wishFulfillments: plan.wishFulfillments.map((fulfillment) => ({
      ...fulfillment,
      selectedProductIds: fulfillment.selectedProductIds.filter((id) => !goneIds.has(id)),
    })),
  });
}

export function setRowQuantity(plan: PartyPlanDraft, row: VerifiedProduct, quantity: number) {
...
export function recalculate(plan: PartyPlanDraft): PartyPlanDraft {
  const totalUah = money(plan.products.reduce((sum, product) => sum + product.lineTotalUah, 0));
  return { ...plan, totalUah };
}

/** Final shape handed back to the web app: totals, per-member coverage placeholders, and a summary line. */
export function finishPlan(plan: PartyPlanDraft | null, memberIds: string[]): PartyPlanDraft {
  const base = recalculate(plan ?? emptyPlan());
  const coverage = Object.fromEntries(memberIds.map((id) => [id, base.coverage[id] ?? { foodGrams: 0, drinkMilliliters: 0 }]));
  const productCount = base.products.length;
  return {
    ...base,
    coverage,
    summary: `${productCount} ${productCount === 1 ? "товар" : productCount >= 2 && productCount <= 4 ? "товари" : "товарів"} на ${base.totalUah} грн`,
  };
}
