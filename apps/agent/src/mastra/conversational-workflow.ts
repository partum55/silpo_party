...
function modeInstructions(mode: Input["mode"], actorId: string, memberIds: string[]) {
  if (mode === "SHOPPING") {
    return `SHOPPING mode: interpret purchase requests as direct product additions, never as recipes. Assign every newly requested product only to actor ${actorId}, and preserve other participants' products.`;
  }
  if (mode === "DINNER") {
    return "DINNER mode: participants request dishes. Use recipes, combine the purchasable ingredients, assign each recipe to its requesters, and never buy pantry staples such as salt, pepper, water, or cooking oil.";
  }
  return `EVENT mode: autonomously plan the complete event for all participants (${memberIds.join(", ")}): main food, sides, drinks, sauces, snacks, and other sensible essentials. Assign shared purchases to everyone and respect the total budget.`;
}
