...
If "restrictions" are given (allergies or diets of the guests), plan only food and drinks that respect them.`;
...
export async function generateChecklist({
  ...
  /** Every guest's restrictions, since the whole party shares an event plan. */
  restrictions: string[];
  llm: Llm;
}) {
  return llm.json(checklistSchema, {
    ...
    role: "smart",
    timeoutMs: MODEL_TIMEOUT_MS.checklist,
    data: {
      brief,
      participantCount,
      budgetUah,
