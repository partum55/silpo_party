// Food-restriction rules: whether a Silpo product is safe for a member's profile restrictions, judged from its
// name, labels, ingredients, and allergens. Candidate selection (turn/resolve-items.ts) applies them.
import type { HydratedProduct } from "./plan.ts";

export type RestrictionResult = "safe" | "unsafe" | "unknown";
...
/** Safety of a product for every restriction at once: any conflict wins, then any missing evidence. */
...
/** The restrictions a product cannot be confirmed safe for, because Silpo lists no composition for it. */
export function unverifiedRestrictions(product: HydratedProduct, restrictions: string[]) {
  return restrictions.filter((restriction) => restrictionSafety(product, restriction) === "unknown");
}
