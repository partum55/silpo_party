...
export type CatalogSession = {
  ...
  /** The session user's food restrictions from their Silpo profile, e.g. ["lactose", "vegan"]. */
  foodRestrictions(): Promise<string[]>;
};
...
export function createCatalogSession({
  ...
  return {
    ...
    foodRestrictions: () => store.getOrLoad(`restrictions:${userId}`, cacheTtl.foodRestrictions, async () =>
...
/** A member's food restrictions from their own Silpo profile (cached; see cacheTtl.foodRestrictions). */
export function readFoodRestrictions(userId: string) {
  return withCatalogSession(userId, (session) => session.foodRestrictions());
}
